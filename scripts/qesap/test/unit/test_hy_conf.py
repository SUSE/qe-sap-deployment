...
tf_var_value_strategy = st.one_of(
    ...
    st.dictionaries(
        ...
        values=st.text(
            max_size=20
        ),  # yaml_to_tfvars_entry for dicts expects string values
        min_size=0,
        max_size=3,
    ),
)
...
def config_maps_without_ansible(draw):
    ...
    structured_parts = {
        ...
        ),  # 5 as we are at apiver:3 and about to add 4
    ...
    for _ in range(num_extra_items):
        extra_key = draw(
            st.text(min_size=1, max_size=15).filter(
                lambda k: k != "ansible" and k not in config
            )
        )
...
@pytest.mark.skipif(
    "FUZZYTEST" not in os.environ, reason="Fuzzy test disabled by default"
)
@example({})
