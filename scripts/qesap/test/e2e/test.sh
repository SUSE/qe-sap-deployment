...
if [ ! -f "${TEST_TERRAFORM_TFVARS}" ]; then
    test_die "Generated file ${TEST_TERRAFORM_TFVARS} not found!"
fi
grep lampone "${TEST_TERRAFORM_TFVARS}" || test_die "${TEST_TERRAFORM_TFVARS} generated from test_2.yaml should contain the world lampone"
...
if [ ! -f "${TEST_ANSIBLE_MEDIA}" ]; then
    test_die "Generated file ${TEST_ANSIBLE_MEDIA} not found!"
fi
grep corniolo "${TEST_ANSIBLE_MEDIA}" || test_die "${TEST_ANSIBLE_MEDIA} generated from test_3.yaml should contain the corniolo"
...
if [ ! -f "${TEST_TERRAFORM_TFSTATE}" ]; then
    test_die "Generated file ${TEST_TERRAFORM_TFSTATE} not found!"
fi
