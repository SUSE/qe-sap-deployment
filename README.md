...
This project aims to provide automation that will create highly-available SAP deployments in the cloud to aid in cluster testing.
...
To mitigate code duplication in the terraform section across multiple different yaml configuration files, the user can use so called Terraform tfvars templates.

The user can create a file like `/home/user/terraform.template.tfvars` that is expected to use same syntax of tfvar files.
...
The Ansible playbooks needs some .yaml configuration files. Some of them are generated by Terraform, some of them have to be provided by the user. The **qesap.py** `configure` command can support the user to create them.
...
ansible:
  sequences:
    create:
      ...
      - pre-cluster.yaml -e use_sapconf=true
...
The Terraform step also generates the Ansible inventory file **inventory.yaml**
